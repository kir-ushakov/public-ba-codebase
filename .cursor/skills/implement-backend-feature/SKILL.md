...
2. `<name>.errors.ts` — an enum of codes plus `<Name>Errors` factories returning `Result.fail(new UseCaseError(...))` with `httpCode`. If the use case cannot fail yet, keep the file with `type XxxError = UseCaseError<never>`.
